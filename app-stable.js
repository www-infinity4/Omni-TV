(function(){
  ...
  const els={shell:$('screenShell'),frame:$('stationFrame'),grid:$('channelGrid'),clock:$('clock'),networkState:$('networkState'),channelLabel:$('channelLabel'),nowTitle:$('nowTitle'),nowMeta:$('nowMeta'),idle:$('idleCard'),loading:$('loadingCard'),loadingChannel:$('loadingChannel'),prev:$('prevChannel'),next:$('nextChannel'),wake:$('wakeChannel'),open:$('openStation'),share:$('shareButton'),wallet:$('walletButton'),search:$('channelSearch'),status:$('status')};
  ...
  function walletState(){try{const session=JSON.parse(localStorage.getItem('starquest_session')||'null'),users=JSON.parse(localStorage.getItem('starquest_users')||'{}');if(session?.key&&users[session.key])return users[session.key];return JSON.parse(localStorage.getItem('starquest_guest_profile_v1')||'{}')}catch{return {}}}
  function renderWallet(){if(!els.wallet)return;const wallet=walletState(),tokens=Math.max(0,Number(wallet.tokens)||0),pending=Math.max(0,Number(wallet.pendingShareCredits)||0);els.wallet.textContent='Wallet '+tokens.toFixed(1)+' ⭐'+(pending?' · '+pending+'/10':'')}
  async function creditShare(reference){if(window.ControlPhi?.ensureShareCredit)return await window.ControlPhi.ensureShareCredit(reference,'web_share_api');return null}
  async function share(){const state=selected?(states.get(selected.slug)||{}):{},title=state.title||selected?.name||'Omni TV',cleanChannelUrl=selected?.url?safeUrl(selected.url):location.origin+location.pathname,payload={title:selected?title+' · '+selected.name:'Omni TV — Live Network Surfer',text:selected?'Watching '+title+' live from '+selected.name+'.':'Flip through the Infinity TV network live on one page.',url:cleanChannelUrl};try{if(navigator.share){await navigator.share(payload);const reward=await creditShare(payload.url);renderWallet();els.status.textContent=reward?.awarded?'Shared · 1 StarCoin completed!':reward?.progressToNextCoin!=null?'Shared · StarCoin progress '+reward.progressToNextCoin+'/10':'Shared. News Phi received this completed share.'}else{await navigator.clipboard.writeText(payload.url);els.status.textContent='Channel link copied. Open Android Share to earn 1/10 StarCoin.'}}catch(e){if(e?.name!=='AbortError')els.status.textContent='Share did not complete.'}}
  ...
  async function start(){
    ...
    channels=await getChannels();renderCards();renderWallet();if(!channels.length){els.status.textContent='Channel registry did not load.';return}if(!fromHash())els.nowMeta.textContent='Choose any channel below. Omni TV will open its live player here.';setInterval(refreshVisible,CACHE_MS)
  }
  els.frame.addEventListener('load',()=>{const generation=tuneId;[0,150,500,1000].forEach(ms=>setTimeout(()=>ready(generation),ms))});overlay.addEventListener('click',wake);els.prev.addEventListener('click',()=>select(selectedIndex<0?channels.length-1:selectedIndex-1));els.next.addEventListener('click',()=>select(selectedIndex<0?0:selectedIndex+1));els.wake.addEventListener('click',wake);els.open.addEventListener('click',()=>selected&&window.open(selected.url,'_blank','noopener'));els.share.addEventListener('click',share);els.wallet?.addEventListener('click',()=>{renderWallet();els.status.textContent='Wallet updated from the shared StarCoin ledger.'});els.search.addEventListener('input',filter);window.addEventListener('hashchange',fromHash);start();
})();
